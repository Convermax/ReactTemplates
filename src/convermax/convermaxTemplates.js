...
class ConvermaxTemplates {
    ...
    components.forEach(component => {
      ...
      if (cmRepeater(component.name)) {
        if (this.repeaterContext.length === 0) {
          ...
          $component.replaceWith(this.repeaterComponent(
            $component,
  ...
  parseReactTemplate(fnString, suffix) {
    ...
    let returnStatement = escodegen.generate(ast.body[0].expression.body.body.find(v => v.type === 'ReturnStatement').argument);

    const scopeArr = [];

    ast.body[0].expression.body.body = ast.body[0].expression.body.body.forEach(v => {
